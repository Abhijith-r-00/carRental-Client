  ...
  const handleSearch = async (e) => {
    ...
    if (sessionStorage.getItem("token")) {
      try {
        ...
        const apiResponse = await getAllAvailableVehicle(location, requestHeader);
    ...
    <>
      ...
      <div className="dashboard">
        ...
        <div className="vehicle-filters">
          <button className={`filter-btn ${activeFilter === 'all' ? 'active' : ''}`} onClick={() => setActiveFilter('all')}>All Vehicles</button>
          <button className={`filter-btn ${activeFilter === 'electric' ? 'active' : ''}`} onClick={() => setActiveFilter('Electric')}>Electric</button>
          <button className={`filter-btn ${activeFilter === 'suv' ? 'active' : ''}`} onClick={() => setActiveFilter('SUV')}>SUVs</button>
          ...
          {filteredVehicles.map(vehicle => (
            <div key={vehicle._id} className="vehicle-card">
              <div className="card-image" style={{ backgroundImage: `url(${vehicle.image})` }}>
                <button
                  className={`favorite-btn ${favorites.includes(vehicle._id) ? 'active' : ''}`}
                  onClick={() => toggleFavorite(vehicle._id)}
                >
                  <FaHeart />
                </button>
                <div className="vehicle-rating"><FiStar /> {vehicle.rating}</div>
              </div>
              ...
              <div className="card-content">
                ...
                <div className="vehicle-specs">
                  <div className="spec-item"><IoMdSpeedometer /><span>{vehicle.transmission}</span></div>
                  <div className="spec-item"><FaCar /><span>{vehicle.seats} seats</span></div>
                  <div className="spec-item"><FaGasPump /><span>{vehicle.fuelType}</span></div>
                  <div className="spec-item"><MdAirlineSeatReclineExtra /><span>{vehicle.mileage} km</span></div>
                  <div className="spec-item">Year: {vehicle.year}</div>
                </div>

                <div className="vehicle-location"><FiMapPin /> {vehicle.location}</div>
                ...
                <div className="card-footer">
                  <div className="price">
                    <span>From</span>
                    <h4>${vehicle.pricePerDay}<small>/day</small></h4>
                  </div>
                  <button className="book-btn" onClick={() => setSelectedVehicle(vehicle)}>
                    Book Now
                  </button>
                </div>
              </div>
